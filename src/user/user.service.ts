...
export class UserService {
    ...
    async delete(nickname: string) {
        return this.userModel.findOneAndUpdate({nickname}, {deleted:true})
    }
    
    async update(nickname: string, toUpdate) {
        await this.userModel.findOneAndUpdate({nickname, deleted: false}, toUpdate)
    ...
    async upvote(id: string, nickname:string): Promise<User> {
        ...
        const donwvoters = user.downvotedBy.filter((element) => element !== id)
        upvoters.push(id)
        const updatedUser = await this.update(nickname, {
            upvotedBy: upvoters,
            donwvoters: donwvoters,
            votes: user.votes + 1
        })
    ...
    async downvote(id: string, nickname: string): Promise<User> {
      ...
      async login(loginNickname: string, loginPassword: string) {
        try {
            ...
            const newHash = (await this.hashService.hashWithProvidedSalt(loginPassword, salt)).hash
